...
public class JourneyList
{
    ...
    public passGroup randomPassGroupGenerator() throws FileNotFoundException, IOException {
    	ArrayList<passGroup> choiceList = new ArrayList<passGroup>();
    	TreeSet<Journey> jlist = journeyListCreator("wah");
    	while (jlist.isEmpty() != true){
    		Journey current = jlist.first();
    		passGroup p = new passGroup(current);
    		choiceList.add(p);
    		jlist.remove(current);
    	}
    	Random rand = new Random();
    	int index = rand.nextInt(choiceList.size());
    	passGroup choice = choiceList.get(index);
    	return choice;
    }
    
    public regNum regNumGenerator() throws FileNotFoundException, IOException {
    	ArrayList<regNum> choiceList = new ArrayList<regNum>();
    	TreeSet<Journey> jlist = journeyListCreator("wah");
    	while (jlist.isEmpty() != true){
    		Journey current = jlist.first();
    		regNum p = new regNum(current);
    		choiceList.add(p);
    		jlist.remove(current);
    	}
    	Random rand = new Random();
    	int index = rand.nextInt(choiceList.size());
    	regNum choice = choiceList.get(index);
    	return choice;
    }
    /**
    ...
    public static void main(String[] args) throws IOException
    {
    	...
    	//System.out.println(test.randomPassGroupGenerator().getPassDest());
    	//System.out.println(test.randomPassGroupGenerator().getPassNumbers());
    	//System.out.println(test.regNumGenerator().getRegistration());
    }
